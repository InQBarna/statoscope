...
public protocol Reducer {
    ...
    /// The generated ChildStores accessor type. Default: EmptyChildStores (no children).
    associatedtype ChildStores: ChildStoresProtocol = EmptyChildStores
    ...
    /// Type-erased descriptors for each `@SubState` property. Generated by `@Reducer` macro.
    static var _childSlots: [AnyChildSlot<State>] { get }
    /// Type-erased descriptors for each `@SuperState` / `@ReducerInjected` property. Generated by `@Reducer` macro.
    static var _superSlots: [AnySuperSlot<State>] { get }
    /// Builds the typed ChildStores accessor from the internal child store cache.
    static func _makeChildStores(from cache: [AnyHashable: AnyObject]) -> ChildStores
}

extension Reducer {
    /// Default: no automatic trigger on wiring
    public static var defaultTrigger: When? { nil }
    /// Default: builds ChildStores from cache using protocol initializer
    public static func _makeChildStores(from cache: [AnyHashable: AnyObject]) -> ChildStores {
        ChildStores(cache: cache)
    }
}

// Reducers with no children get empty defaults automatically.
extension Reducer where ChildStores == EmptyChildStores {
